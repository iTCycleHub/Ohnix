...
const CategoryUnit = () => {
    ...
    const [viewCategoryModalVisible, setViewCategoryModalVisible] =
        useState(false);
    ...
    const updateStats = (type, data) => {
        setStats((prev) => {
            if (type === "categories") {
                ...
                const userUnits = data.filter(
                    (unit) => unit.created_by === user?._id
                );
                return {
                    ...prev,
    ...
    const handleUnitSubmit = async (values) => {
        const loadingToast = toast.loading(
            editingUnit ? "Updating unit..." : "Creating unit..."
        );
        try {
            if (editingUnit) {
                // Update unit
    ...
    const categoryColumns = [
        ...
        },
        {
            ...
            render: (createdBy) => (
                <Tag
                    color={createdBy === user?._id ? "green" : "blue"}
                    icon={<UserOutlined />}
                >
                    ...
                    <Form.Item
                        ...
                        rules={[
                            ...
                            },
                            {
                                min: 1,
                                message:
                                    "Unit name must be at least 1 character",
                            },
                            {
                                max: 20,
                                message:
                                    "Unit name cannot exceed 20 characters",
                            },
                        ]}
                    >
                        <Input
            ...
            >
                {viewingUnit && (
                    <div className="space-y-4 mt-4">
                        <div className="bg-gray-50 p-4 rounded-lg">
                            <Row gutter={[16, 16]}>
                                <Col span={24}>
                                    <div className="flex justify-between items-center">
                                        <Title level={4} className="mb-0">
                                            {viewingUnit.unit_name}
                                        </Title>
                                        <Tag
                                            color={
                                                viewingUnit.created_by ===
                                                user?._id
                                                    ? "green"
                                                    : "blue"
                                            }
                        ...
                        <div className="flex justify-end space-x-2">
                            {canEditUnit(viewingUnit) && (
                                <Button
                                    type="primary"
                                    icon={<EditOutlined />}
                                    onClick={() => {
                                        setViewUnitModalVisible(false);
                                        openUnitModal(viewingUnit);
                                    }}
                                >
                                    Edit Unit
                                </Button>
                            ...
                            <Button
                                onClick={() => setViewUnitModalVisible(false)}
                            >
                                Close
                            </Button>
                        </div>
